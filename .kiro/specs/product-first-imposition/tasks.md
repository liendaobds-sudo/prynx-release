...
- [x] 1. Module lõi `ProductAdvisor.recommendInNhanh` (thuần toán, không UI/PDF)
...
- [x] 2. Unit test cho `recommendInNhanh` (khoá Correctness Properties)
...
- [x] 3. `applyRecommendation` → đổ vào store + test
...
- [x] 4. UI `ProductFirstPanel` + cắm vào dashboard (giữ Nâng cao)
  - Component `ProductFirstPanel.tsx` (chọn sản phẩm + khổ in_nhanh + option-card
    + áp dụng 1 chạm) + smoke test.
  - Đã cắm vào `ImposerDashboard.tsx`: toggle "✨ Đề xuất theo sản phẩm" đầu panel
    Booklet, gate `taskMode==='booklet' && paperClassification==='in_nhanh' && showProductFirst`,
    truyền `pageCount=sourceTotalPages` + finished dims từ `sourcePageDim`
    (pt×0.352778→mm); apply → ẩn toggle, hiện section kỹ thuật. Commit `db81064`.
