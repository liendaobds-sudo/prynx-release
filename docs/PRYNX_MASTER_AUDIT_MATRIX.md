...
| **W6 — Multi-tab, routing, event, state ownership** | `STALE` | Audit NAV.1–NAV.12 từng đạt full Vitest; audit mở file từng có smoke Tauri thật. `W6-U02` Recipe đã hoàn tất Lô R1: working artifact, cancel outcome và commit tuần tự đạt `AUTO`; 128 test mục tiêu và typecheck đạt. | `App`, Home, Imposition, tool registry, recovery và entitlement đã tiếp tục đổi. Recipe còn 2 P0 ở page state/multi-output; chưa có record→save→play PDF artifact hay runtime Tauri. Ownership recorder theo tab, Undo và nhiều contract tool vẫn mở. Chưa lặp đầy đủ multi-tab/current-tool, background listeners, native drop, recovery, USB/UNC/NAS và đủ định dạng. | Runtime smoke `W6-U02` Lô R1 rồi duyệt R2A/R2B; đồng thời giữ `W6-U01` cho hai tab cùng loại + tab nền + native drop + đóng/reopen/recovery. |
...
| 8a | `W6-U02` | Ghi → lưu → phát Recipe trên working file và nhiều tab | Step đúng thao tác/tab; output bước N là input N+1; path/bytes có ownership; Hủy/Undo không chạy tiếp; CRUD sống qua restart | `TRACED`; Lô R1 đạt `AUTO` cho `§PLAY.1/.2/.6/.7` với 128 test mục tiêu + typecheck. Còn 2 P0 (`§PLAY.3/.4`), recorder ownership/Undo, integration PDF artifact và runtime Tauri. |
