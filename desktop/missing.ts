    ...
    const gableFold = params.gableStyle === 'pitched' ? 30 : 90;
    const handleFold = params.gableStyle === 'pitched' ? -30 : -90;
    ...
    const Z_height = h1 * Math.sin(gableFold * Math.PI / 180); // Chiều cao thực tế Z
    let sideFlapFold = gableFold;
    if (Z_height > 0.001) {
        sideFlapFold = Math.atan2(insetA, Z_height) * 180 / Math.PI;
    } else {
        sideFlapFold = 90; // for flat roof
    }

    // --- C3. Side Flap trái (trên SideA Panel, W wide) — Tam giác ---
    const sfLResult = buildSideTriFlap(xSideAL, xSideAR, yTop, W, L, 1, params.HFH, params.gableStyle, params.SLW, params.SLH, params.TRW);
    allPaths.push(...sfLResult.paths);
    panels.push({
        ...
        holes: [tracePerimeter(sfLResult.holes)],
        annotations: sfLResult.annotations,
        foldPhase: [0.6, 0.9],
    });

    // --- C4. Side Flap phải (trên SideB Panel, W wide) — Tam giác ---
    const sfRResult = buildSideTriFlap(xSideBL, xSideBR, yTop, W, L, 1, params.HFH, params.gableStyle, params.SLW, params.SLH, params.TRW);
    allPaths.push(...sfRResult.paths);
    panels.push({
        ...
        foldAngle: sideFlapFold, foldDirection: -1,
        holes: [tracePerimeter(sfRResult.holes)],
