...
import type { SpreadFoldPattern } from './FoldPatterns';
...
// ==================== SPREAD PLACER SERIALIZER ====================

/**
 * Serialize a SpreadPlacer rendering plan (FoldPattern grid) into JSON.
 * This replaces the role of SpreadPlacer.ts for Backend execution.
 */
export function serializeSpreadPlacerPlan(
    spreadCount: number,
    spreadW: number,
    spreadH: number,
    ...
    const markLenPt = ((settings as any).markLength ?? 5) * MM_TO_POINTS;
    const markOffPt = ((settings as any).markOffset ?? 3) * MM_TO_POINTS;
    const markThickPt = ((settings as any).markThickness ?? 0.25) * MM_TO_POINTS;
    const gapXPt = (settings.gapX || 0) * MM_TO_POINTS;
    const gapYPt = (settings.gapY || 0) * MM_TO_POINTS;
    const isEven = ((settings as any).spreadDistribution || 'clustered') === 'even';

    const { cols, rows } = pattern;
    const usableW = sheetWPt - marginLeftPt - marginRightPt;
    const usableH = sheetHPt - gripperPt - marginTopPt;
    const totalGridW = isEven ? usableW : cols * spreadW + (cols > 1 ? (cols - 1) * gapXPt : 0);
    const totalGridH = rows * spreadH + (rows > 1 ? (rows - 1) * gapYPt : 0);
    const gridOriginX = marginLeftPt + (usableW - totalGridW) / 2;
    const gridOriginY = gripperPt + (usableH - totalGridH) / 2;

    const actualGridW = cols * spreadW + (cols > 1 ? (cols - 1) * gapXPt : 0);
    const actualGridH = totalGridH;

    const gridRatio = actualGridW / actualGridH;
    const sheetRatio = usableW / usableH;
    let isRotated = false;
    
    // Auto-detect if rotation is needed to fit the paper
    if (gridRatio <= 1 && sheetRatio >= 1 && actualGridH > usableH) {
        isRotated = true;
    } else if (gridRatio > 1 && sheetRatio < 1 && actualGridW > usableW) {
        isRotated = true;
    }

    const cx = marginLeftPt + usableW / 2;
    const cy = gripperPt + usableH / 2;

    const spreadsPerSig = pattern.spreadsPerSig;
    const totalSigs = Math.ceil(spreadCount / spreadsPerSig);
    const plateSides: ('front' | 'back')[] = pattern.backPlate.length > 0 ? ['front', 'back'] : ['front'];
    ...
    for (let sig = 0; sig < totalSigs; sig++) {
        ...
        for (const plateSide of plateSides) {
            ...
            for (const slot of slots) {
                ...
                if (tempPageIdx >= spreadCount) continue;

                let cellX = 0;
                if (isEven) {
                    const cellW = cols > 0 ? usableW / cols : usableW;
                    cellX = gridOriginX + slot.col * cellW + (cellW - spreadW) / 2;
                } else {
                    cellX = gridOriginX + slot.col * (spreadW + gapXPt);
                }
                const cellY = gridOriginY + (rows - 1 - slot.row) * (spreadH + gapYPt);

                placements.push({
                    source_page: tempPageIdx,
                    x_pt: cellX,
                    y_pt: cellY,
                    rotation_deg: slot.rotation,
                    scale: 1.0,
                    creep_offset_pt: 0,
                    clip: { x_pt: cellX, y_pt: cellY, w_pt: spreadW, h_pt: spreadH },
                    native_angle: 0,
                });
            }

            // Marks for this plate
            const marks: MarkInstruction[] = [];
            // (Marks serialization follows same logic as SpreadPlacer.ts drawTrimMarks/drawFoldMarks)

            const side: SheetSideInstruction = { placements, marks };
            
            if (isRotated) {
                applyGridRotationToSide(side, cx, cy);
            }
            
            // Find or create the sheet entry
            const sheetIdx = sig * plateSides.length + (plateSide === 'back' ? 1 : 0);
            if (plateSide === 'front') {
                sheets.push({
                    ...
                    height_pt: sheetHPt,
                    front: side,
                    back: { placements: [], marks: [] },
                });
            } else {
                // Attach to the last sheet
                sheets[sheets.length - 1].back = side;
            }
        }
    }
    ...
    return {
        ...
        global: {
            bleed_pt: bleedPt,
            paper_thickness_pt: 0,
            total_source_pages: totalSourcePages,
            mark_type: (settings as any).markType || 'none',
            mark_length_pt: markLenPt,
            mark_offset_pt: markOffPt,
            mark_thickness_pt: markThickPt,
        },
