...
// ── Seal Flap ngang (cho bì thư dọc — nắp mở sang phải) ──
// Giống buildSealFlap nhưng xoay 90°: seal mở theo trục X
function buildSealFlapH(
    yBot: number, yTop: number,
    sealBaseX: number, flapH: number,
    shape: 'straight' | 'pointed' | 'rounded',
): PathSegment[] {
    const h = snap(yTop - yBot);
    const xEnd = snap(sealBaseX + flapH);
    const paths: PathSegment[] = [];

    switch (shape) {
        case 'straight': {
            const sideInset = snap(flapH * Math.tan(10 * Math.PI / 180));
            const sideR = snap(Math.min(5, flapH * 0.2));

            const pB0 = pt(sealBaseX, yBot);
            const pBtip = pt(xEnd, snap(yBot + sideInset));
            const pTtip = pt(xEnd, snap(yTop - sideInset));
            const pT0 = pt(sealBaseX, yTop);

            const fB = smoothFillet(pBtip, pB0, pTtip, sideR);
            const fT = smoothFillet(pTtip, pBtip, pT0, sideR);

            paths.push(
                line(pB0, fB.t1, 'CUT'),
                fB.seg,
                line(fB.t2, fT.t1, 'CUT'),
                fT.seg,
                line(fT.t2, pT0, 'CUT'),
            );
            break;
        ...
        case 'pointed': {
            const cy = snap((yBot + yTop) / 2);
            const tipR = snap(Math.min(3, flapH * 0.08));
            const sideH = 28;
            const sideInset = snap(sideH * Math.tan(10 * Math.PI / 180));
            const sideR = 5;

            const pB0 = pt(sealBaseX, yBot);
            const pB1 = pt(snap(sealBaseX + sideH), snap(yBot + sideInset));
            const pB2 = pt(xEnd, snap(cy - tipR));
            const pT2 = pt(xEnd, snap(cy + tipR));
            const pT1 = pt(snap(sealBaseX + sideH), snap(yTop - sideInset));
            const pT0 = pt(sealBaseX, yTop);

            const fB = smoothFillet(pB1, pB0, pB2, sideR);
            const fT = smoothFillet(pT1, pT2, pT0, sideR);
            ...
            paths.push(
                ...
                line(fB.t2, pB2, 'CUT'),
                bezierSegment(
                    pt(xEnd, snap(cy - tipR)),
                    pt(snap(xEnd + tipR * 0.2), snap(cy - tipR * 0.3)),
                    pt(snap(xEnd + tipR * 0.2), snap(cy + tipR * 0.3)),
                    pt(xEnd, snap(cy + tipR)),
                    'CUT',
                ),
        ...
        case 'rounded': {
            const halfH = h / 2;
            const r = snap((halfH * halfH + flapH * flapH) / (2 * flapH));
            const cy = snap((yBot + yTop) / 2);
            const cxR = snap(sealBaseX + flapH - r);

            const halfAngle = Math.asin(Math.min(1, halfH / r));
            const startDeg = -halfAngle * 180 / Math.PI;
            const endDeg = halfAngle * 180 / Math.PI;
            paths.push(
                arcToBezier(cxR, cy, r, startDeg, endDeg, 'CUT'),
            );
            break;
        }
    }
