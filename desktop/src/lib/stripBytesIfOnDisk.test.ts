...
function fileWithPath(
    ...
    const f: RuntimeMetadataFile = new File([buffer], name, { type: 'application/pdf' });
...
describe('stripBytesIfOnDisk', () => {
    it('Tauri + có path → không giữ bytes nhưng vẫn giữ kích thước logic', () => {
        ...
        expect(light.path).toBe('C:/tmp/doc.pdf');
    });
    ...
    it('giữ cờ __editCommit khi có', () => {
        ...
        const light = stripBytesIfOnDisk(orig);
        expect(light.__editCommit).toBe(true);
    });
    ...
    it('không set __editCommit nếu file gốc không có', () => {
        ...
        const light = stripBytesIfOnDisk(orig);
        expect(light.__editCommit).toBeUndefined();
    });
    ...
    it('giữ metadata runtime quyết định luồng lưu và render', () => {
        ...
        Object.defineProperty(orig, '__prynxArtifactLeaseToken', { value: 'a'.repeat(64), configurable: true });

        const light = stripBytesIfOnDisk(orig);

        expect(light.__editCommit).toBe(true);
        expect(light.isGenerated).toBe(true);
        expect(light.isTempUploadPath).toBe(true);
        expect(light.__pathMaterializationFailed).toBe(true);
        expect(light.__prynxArtifactLeaseToken).toBe('a'.repeat(64));
    });

    it('Tauri nhưng KHÔNG path → giữ nguyên file (fallback bytes)', () => {
        setTauri(true);
        const orig = new File([new ArrayBuffer(3000)], 'blob.pdf', { type: 'application/pdf' });
