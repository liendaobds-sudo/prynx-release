...
export default function CoverNumberingTool({
    ...
    const validationMsg = useMemo(() => {
        if (derived.error === 'bad_range') return '⚠️ Dải số hoặc số cuốn không hợp lệ.';
        if (derived.error === 'too_large') return '⚠️ Dải số quá lớn (vượt giới hạn an toàn).';
    ...
    const handleGenerate = async () => {
        try {
            if (!derived.valid) throw new Error('Cấu hình chưa hợp lệ — xem cảnh báo.');
            if (clusters.length === 0) throw new Error('Hãy kéo ít nhất 1 cụm bìa ({X}/{Y}/{Z}) vào PDF.');
            if (!pdfFile) throw new Error('Chưa có file PDF gốc.');
            setBusy(true);
            setStatus('Đang tính kế hoạch đánh số bìa...');
            ...
            if (roleOf.size === 0) throw new Error('Chưa có trường {X}/{Y}/{Z} nào. Hãy kéo các chip ở mục 2 vào bìa.');
            ...
            if (singleFileMode) {
                if (coverPageIdx.length === 0)
                    throw new Error('Dải trang bìa không hợp lệ. Nhập số trang bìa (vd: 1 hoặc 1-2).');
                setStatus('Đang trích trang bìa khỏi file...');
                const { PDFDocument } = await import('pdf-lib');
    ...
    return (
        <div className="flex flex-col h-full bg-white dark:bg-zinc-900 border-l border-slate-200 dark:border-zinc-800 p-4 gap-4 overflow-y-auto scroller-thin">
            <div className="flex items-center gap-2 pb-3 border-b border-slate-200 dark:border-zinc-700 shrink-0">
                <button onClick={onBack} className="p-1.5 hover:bg-slate-100 dark:hover:bg-zinc-800 rounded-md text-slate-500" title="Quay lại">
                    <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 19l-7-7m0 0l7-7m-7 7h18" /></svg>
                </button>
                <div className="flex-1 text-center pr-8">
                    <h2 className="text-sm font-bold text-slate-800 dark:text-white uppercase tracking-wider flex items-center justify-center gap-2"><span>🔖</span><span>MẸC BÌA (CHẠY SỐ BÌA)</span></h2>
            ...
            <div className="shrink-0 space-y-3">
                <div className="flex items-center justify-between">
                    <span className="text-sm font-bold text-slate-800 dark:text-zinc-200">1. Cấu hình bìa</span>
                    <label className="flex items-center gap-1.5 text-[11px] text-slate-600 dark:text-zinc-300 cursor-pointer" title="Dùng chung dải số với Mẹc Số (ruột) — hai tab luôn khớp.">
                ...
                <div className="p-3 border border-slate-200 dark:border-zinc-700 rounded-lg space-y-3 bg-slate-50/50 dark:bg-zinc-800/20">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                        <label className="flex flex-col gap-1 text-[10px] font-medium text-slate-500">Kiểu đánh ruột
                            <select value={v.innerMode} onChange={e => setV({ innerMode: e.target.value as InnerMode })} className="h-8 px-1 text-xs border border-slate-300 dark:border-zinc-600 rounded">
                                ...
                                <option value="reset">Reset mỗi cuốn</option>
                            </select>
                        </label>
                        <label className="flex flex-col gap-1 text-[10px] font-medium text-slate-500">Chế độ chạy
                            <select value={v.distribution} onChange={e => setV({ distribution: e.target.value as Distribution })} className="h-8 px-1 text-xs border border-slate-300 dark:border-zinc-600 rounded">
                                <option value="stack">Cắt chồng</option>
                                <option value="sequential">Tuần tự</option>
                            </select>
                        </label>
                        <label className="flex flex-col gap-1 text-[10px] font-medium text-slate-500">Kiểu xếp
            ...
            <div className="shrink-0 space-y-2">
                <span className="text-sm font-bold text-slate-800 dark:text-zinc-200">2. Kéo trường vào mẫu bìa</span>
                <p className="text-[11px] text-slate-500">Kéo 3 trường vào mỗi ô bìa rồi Group lại thành 1 cụm.</p>
                <div className="grid grid-cols-3 gap-2">
                    <div onPointerDown={e => startVdpDrag(e, 'text', '{X} Số cuốn', '{X}')} className="bg-indigo-50 border-2 border-indigo-200 dark:bg-indigo-900/20 dark:border-indigo-800 p-2 rounded cursor-grab text-center text-xs font-bold text-indigo-700 dark:text-indigo-300">{'{X}'} Số cuốn</div>
                    <div onPointerDown={e => startVdpDrag(e, 'text', '{Y} Ruột đầu', '{Y}')} className="bg-sky-50 border-2 border-sky-200 dark:bg-sky-900/20 dark:border-sky-800 p-2 rounded cursor-grab text-center text-xs font-bold text-sky-700 dark:text-sky-300">{'{Y}'} Ruột đầu</div>
                    <div onPointerDown={e => startVdpDrag(e, 'text', '{Z} Ruột cuối', '{Z}')} className="bg-rose-50 border-2 border-rose-200 dark:bg-rose-900/20 dark:border-rose-800 p-2 rounded cursor-grab text-center text-xs font-bold text-rose-700 dark:text-rose-300">{'{Z}'} Ruột cuối</div>
                </div>
                <div className="flex gap-2 items-center text-[11px]">
                    ...
                    {selectedFieldIds.length > 0 && <button onClick={deleteSelectedField} className="text-red-500 px-2 py-1 rounded bg-red-50 dark:bg-red-500/10">Xóa</button>}
                </div>
            </div>

            {/* Nguồn bìa (PA2: 1 file gán trang) */}
            <div className="shrink-0 space-y-2">
                <span className="text-sm font-bold text-slate-800 dark:text-zinc-200">3. Nguồn trang bìa</span>
                <label className="flex items-center gap-2 text-[11px] text-slate-600 dark:text-zinc-300 cursor-pointer">
                    ...
                    Bìa & ruột nằm chung 1 file — gán trang bìa
                </label>
                {singleFileMode && (
            ...
            <div className="mt-auto shrink-0 space-y-2">
                <label className="flex items-center gap-2 text-[11px] text-slate-600 dark:text-zinc-300">
                    <input type="checkbox" checked={spawnNewTab} onChange={e => setSpawnNewTab(e.target.checked)} /> Mở kết quả ở tab mới
                </label>
                <button onClick={handleGenerate} disabled={busy || !derived.valid || clusters.length === 0 || (singleFileMode && coverPageIdx.length === 0)}
                    ...
                    {busy ? 'Đang xử lý...' : `Tạo bìa (${derived.valid ? v.bookletCount : 0} cuốn)`}
                </button>
