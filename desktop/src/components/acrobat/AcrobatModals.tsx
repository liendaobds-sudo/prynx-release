        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4 font-sans">
            <div className="bg-white dark:bg-[#1e1e1e] w-[380px] rounded-xl shadow-2xl flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200 border border-black/5 dark:border-white/10">
                <div className="flex p-6">
                    ...
                    <div>
                        <h3 className="font-semibold text-lg text-slate-800 dark:text-zinc-100 mb-1">Xóa trang?</h3>
                        <p className="text-sm text-slate-600 dark:text-zinc-400">
                            Bạn có chắc chắn muốn xóa <span className="font-bold text-red-600 dark:text-red-400">{selectedCount}</span> trang khỏi tài liệu này không? Hành động này có thể hoàn tác bằng Ctrl+Z.
                        </p>
                    </div>
                </div>
                <div className="flex bg-slate-50 dark:bg-[#151515] px-6 py-4 justify-end gap-3 border-t border-black/5 dark:border-white/5">
                    <button onClick={onClose} className="px-5 h-[38px] flex items-center justify-center rounded font-medium text-[13px] text-slate-700 dark:text-zinc-300 hover:bg-slate-100 dark:hover:bg-white/10 border border-transparent hover:border-slate-200 dark:hover:border-white/10 transition-colors focus:ring-2 focus:ring-slate-400 outline-none min-w-[90px]">Hủy bỏ</button>
                    <button onClick={onConfirm} className="px-6 h-[38px] flex items-center justify-center rounded font-medium text-[13px] bg-red-600 hover:bg-red-700 text-white shadow-sm min-w-[120px] transition-colors outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 dark:focus:ring-offset-[#1e1e1e]">Đồng ý xóa</button>
                </div>
            </div>
        </div>
    );
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4 overflow-y-auto font-sans">
            <div className="bg-white dark:bg-[#1e1e1e] w-[420px] rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200 border border-black/5 dark:border-white/10">
                <div className="flex items-center justify-between px-6 py-4 border-b border-black/5 dark:border-white/5">
                    <h3 className="font-semibold text-base text-slate-800 dark:text-zinc-100 tracking-wide">Xóa Trang (Delete Pages)</h3>
                ...
                <div className="p-6 flex flex-col gap-6 text-[14px]">
                    <div className="flex flex-col gap-3">
                        <label className="text-slate-600 dark:text-zinc-300 font-medium">Chọn luồng cần xóa</label>
                        <div className="flex flex-col gap-3 p-4 bg-slate-50 dark:bg-black/20 rounded-xl border border-slate-100 dark:border-white/5">
                             <label className="flex items-center gap-3 cursor-pointer group">
                                ...
                                <span className="text-slate-700 dark:text-zinc-300">Những trang đang được chọn (Selected)</span>
                            </label>
                            <div className="flex items-center gap-3">
                                <label className="flex items-center gap-3 cursor-pointer group">
                                    ...
                                    <span className="text-slate-700 dark:text-zinc-300 whitespace-nowrap">Theo số trang:</span>
                                </label>
                ...
                <div className="flex justify-end gap-3 px-6 py-4 bg-slate-50 dark:bg-black/20 border-t border-black/5 dark:border-white/5">
                    ...
                    <button className="px-6 h-[38px] flex items-center justify-center rounded font-medium text-[13px] bg-red-600 hover:bg-red-700 text-white shadow-sm min-w-[120px] transition-colors outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 dark:focus:ring-offset-[#1e1e1e]" onClick={() => onConfirm(advDeleteRange, advDeleteFrom, advDeleteTo)}>Đồng ý Xóa</button>
                </div>
            </div>
        </div>
    );
...
export function ExtractPagesModal({ pageCount, initialPagesStr, onConfirm, onClose }: { pageCount: number; initialPagesStr: string; onConfirm: (pagesStr: string, deleteAfter: boolean) => void; onClose: () => void }) {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4 overflow-y-auto font-sans">
            <div className="bg-white dark:bg-[#1e1e1e] w-[420px] rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200 border border-black/5 dark:border-white/10">
                <div className="flex items-center justify-between px-6 py-4 border-b border-black/5 dark:border-white/5">
                    <h3 className="font-semibold text-base text-slate-800 dark:text-zinc-100 tracking-wide">Trích xuất trang</h3>
                ...
                <div className="p-6 flex flex-col gap-6 text-[14px]">
                    <div className="flex flex-col gap-2">
                        <label className="text-slate-600 dark:text-zinc-300 font-medium">Trang cần trích xuất</label>
                        <div className="flex flex-col gap-3 p-4 bg-slate-50 dark:bg-black/20 rounded-xl border border-slate-100 dark:border-white/5">
                            <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col gap-2">
                        <label className="text-slate-600 dark:text-zinc-300 font-medium">Tùy chọn bổ sung</label>
                        <div className="flex flex-col gap-3 p-4 bg-slate-50 dark:bg-black/20 rounded-xl border border-slate-100 dark:border-white/5">
                            <label className="flex items-center gap-3 cursor-pointer group">
                                ...
                                <span className="text-slate-700 dark:text-zinc-300">Xóa các trang này khỏi file gốc sau khi trích xuất</span>
                            </label>
                        </div>
                    </div>
                </div>
                <div className="flex justify-end gap-3 px-6 py-4 bg-slate-50 dark:bg-black/20 border-t border-black/5 dark:border-white/5">
                    <button className="px-5 h-[38px] flex items-center justify-center rounded font-medium text-[13px] text-slate-700 dark:text-zinc-300 hover:bg-slate-100 dark:hover:bg-white/10 border border-transparent hover:border-slate-200 dark:hover:border-white/10 transition-colors focus:ring-2 focus:ring-slate-400 outline-none min-w-[90px]" onClick={onClose}>Hủy bỏ</button>
                    <button className="px-6 h-[38px] flex items-center justify-center rounded font-medium text-[13px] bg-blue-600 hover:bg-blue-700 text-white shadow-sm min-w-[120px] transition-colors outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:focus:ring-offset-[#1e1e1e]" onClick={() => onConfirm(extractPagesStr, extractDeleteAfter)}>Trích xuất</button>
                </div>
            </div>
        </div>
    );
