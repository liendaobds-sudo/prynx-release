...
$failures = @()
try {
    $appExe = Get-ChildItem -LiteralPath $installDir -Filter "PrynX.exe" -File -Recurse -ErrorAction SilentlyContinue |
        Select-Object -First 1
    if (-not $appExe) {
        throw "Khong thay PrynX.exe trong cay da cai: $installDir"
    }
    ...
    $noticeHits = @(Get-ChildItem -LiteralPath $installDir -Recurse -File -Filter "*NOTICE*" -ErrorAction SilentlyContinue |
        Select-String -Pattern "Ghostscript|Artifex|AGPL" -SimpleMatch -List)
