...
def test_real_file_case_picks_spot_stroke_over_bigger_fill():
    ...
    chosen, by_spot = _select_from_paths([bg, die], PAGE, CFG.die_channel_names,
...
def test_extra_artwork_stroke_does_not_steal():
    ...
    chosen, by_spot = _select_from_paths([art_stroke, die], PAGE, CFG.die_channel_names,
                                         CFG.die_colors, CFG.die_color_tol)
    assert chosen is die
    assert by_spot is True
...
def test_no_signal_falls_back_to_largest_stroke():
    ...
    chosen, by_spot = _select_from_paths([s1, s2], PAGE, (), (), 0.06)
...
def test_channel_name_dominates_spot():
    ...
    chosen, by_spot = _select_from_paths([spot_other, cut], PAGE, CFG.die_channel_names,
                                         CFG.die_colors, CFG.die_color_tol)
