...
def test_quantities_are_exact_without_materializing_ratio_as_zones(quantities):
    plan = build_mixed_guillotine_plan(
        [
    ...
    assert totals[0]["actualQuantity"] == quantities[0]
    assert totals[1]["actualQuantity"] == quantities[1]
    assert totals[0]["excessQuantity"] == 0
    assert totals[1]["excessQuantity"] == 0
