...
def _py_solve_optimal_sticker_layout_impl(usable_w: float, usable_h: float, item_w: float, item_h: float, gap_x: float, gap_y: float, strategy: str = 'grid', p5_params: Dict[str, float] = None, p6_params: Dict[str, float] = None, p5_row_params: Dict = None, p6_row_params: Dict = None, p5_col_params: Dict = None, p6_col_params: Dict = None, shape_type: str = 'CUSTOM', shape_props: Dict[str, Any] = None, base_poly: Any = None) -> Dict[str, Any]:
    ...
    if strategy == 'optimal_auto':
        ...
        if shape_type in ('HAMMER', 'DUMBBELL'):
            ...
            # Grid an toÃ n (khÃ´ng bao giá» Ä‘Ã¨)
            ...
            # Head-to-tail vá»›i NFP params (lá»“ng ghÃ©p chÃ­nh xÃ¡c, Ä‘Ã£ tÃ­nh báº±ng binary search)
            ...
            # Row/Col alternating náº¿u cÃ³ params
            if p5_row_params:
            ...
            # === COLLISION VALIDATION trÃªn Táº¤T Cáº¢ candidates ===
            # Skip hex_tiling/staggered â€” these use deterministic geometric formulas
            ...
            if base_poly is not None:
                for cfg, rot, strat in configs:
                    ...
                    if cfg['totalItems'] < original_n:
                        logger.warning(f"[COLLISION_PRE_SORT] {strat} rot={rot}: {original_n} â†’ {cfg['totalItems']} items (loáº¡i {original_n - cfg['totalItems']})")
            ...
            p1 = solve_grid_layout(usable_w, usable_h, item_w, item_h, gap_x, gap_y)
            p2 = solve_grid_layout(usable_w, usable_h, item_h, item_w, gap_x, gap_y)
            p7 = solve_l_shape_layout(usable_w, usable_h, item_w, item_h, gap_x, gap_y)
...
# Give a slight bonus to head_to_tail if total items are the same
        ...
        # HAMMER/DUMBBELL/TRAPEZOID: collision validation Ä‘Ã£ cháº¡y trÃªn táº¥t cáº£ candidates TRÆ¯á»šC sorting
        # â†’ winner Ä‘Ã£ Ä‘Æ°á»£c Ä‘áº£m báº£o collision-free â†’ early return (skip L-shape filler)
        if shape_type in ('HAMMER', 'DUMBBELL', 'TRAPEZOID', 'PARALLELOGRAM'):
        ...
        split_gap = max(gap_x, gap_y, 14.17)  # ~5mm gap between main and fill blocks
