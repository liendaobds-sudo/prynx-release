...
def _py_solve_illustrator_parallelogram_layout(usable_w: float, usable_h: float, item_w: float, item_h: float, gap_x: float, gap_y: float, shape_props: Dict[str, Any] = None) -> Dict[str, Any]:
    ...
    # 4 passes: (orig/rot) Ã— (interlock X / interlock Y)
...
def _py_solve_l_shape_layout(usable_w: float, usable_h: float, item_w: float, item_h: float, gap_x: float, gap_y: float) -> Dict[str, Any]:
    """
    ...
    """
    split_gap = max(gap_x, gap_y, 14.17)  # ~5mm minimum gap between main and fill blocks
