...
def resolve_layout_collisions(items, base_poly, gap_pt):
    ...
    try:
        ...
        _logger.warning(f"[COLLISION_RESOLVER] poly_w={poly_w:.1f} poly_h={poly_h:.1f} area={sticker_area:.1f} gap={gap_pt:.1f} items={len(items)}")
        ...
        gap_tol = max(0.0, gap_pt - 1.5)  # tolerance lỏng hơn (1.5mm) để bù sai số đo Overhang tự động
        overlap_area_threshold = max(1.0, sticker_area * 0.001)
        ...
        if not violations:
            _logger.warning(f"[COLLISION_RESOLVER] Không phát hiện vi phạm — giữ nguyên {len(items)} tem")
            return items
        
        _logger.warning(f"[COLLISION_RESOLVER] Phát hiện {len(violations)} cặp vi phạm (overlap/too-close)")
        for idx_i, idx_j, sev in violations[:10]:
            it_i = items[idx_i]
            it_j = items[idx_j]
            _logger.warning(f"  [{idx_i}]({it_i['x']:.0f},{it_i['y']:.0f}) x [{idx_j}]({it_j['x']:.0f},{it_j['y']:.0f}) severity={sev:.2f}")
        ...
        _logger.warning(f"[COLLISION_RESOLVER] Pruned {len(removed)} items, remaining {len(valid_items)}/{len(items)}")
        return valid_items
