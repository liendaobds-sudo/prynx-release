...
def _xobject_form_geometry(
    page, name: str
) -> tuple[list[float], list[float]] | None:
    """
    Read the drawable geometry of a Form XObject.

    A Form's ``/BBox`` is expressed in the form's own user space. The form
    ``/Matrix`` is applied first, followed by the CTM at the page-level
    ``Do`` operator (the caller performs the latter composition). PDF files
    generated by Illustrator/InDesign frequently store the BBox with reversed
    edges, so the bbox is deliberately derived from all four transformed
    corners instead of assuming x0 <= x1 / y0 <= y1.

    ``None`` is returned for malformed/missing BBox values or a malformed
    Matrix. This is an intentional fail-safe: an unverifiable Form must not
    become an editable candidate by guessing its geometry.
    """
    pg = page if isinstance(page, pikepdf.Page) else pikepdf.Page(page)
    try:
        resources = pg.obj.get("/Resources")
        ...
        if xobjects is None:
            return None
        ref = xobjects.get(pikepdf.Name("/" + name))
        if ref is None or str(ref.get("/Subtype")) != "/Form":
            return None

        raw_bbox = ref.get("/BBox")
        if raw_bbox is None or len(raw_bbox) != 4:
            return None
        bbox_values = [_as_float(value) for value in raw_bbox]
        if not all(
        ...
        raw_matrix = ref.get("/Matrix")
        if raw_matrix is None:
            # PDF's default Form matrix is the identity. A missing Matrix is
            # therefore valid; a present but malformed Matrix is not.
            form_matrix = list(IDENTITY)
        else:
            if len(raw_matrix) != 6:
                return None
            matrix_values = [_as_float(value) for value in raw_matrix]
            if not all(
                value is not None and math.isfinite(float(value))
                for value in matrix_values
            ):
                return None
            form_matrix = [float(value) for value in matrix_values]

        return [float(value) for value in bbox_values], form_matrix
    except Exception:  # noqa: BLE001 - malformed resource -> skip safely
        return None
...
_XOBJECT_WRAPPER_NONPAINTING_OPS = {
    # Graphics-state and coordinate setup.
    "q", "Q", "cm", "gs", "w", "J", "j", "M", "d", "ri", "i",
    "CS", "cs", "SC", "SCN", "sc", "scn", "G", "g", "RG", "rg", "K", "k",
    # Text-state setters sometimes emitted by design tools before an XObject Do.
    "BT", "ET", "Tc", "Tw", "Tz", "TL", "Tf", "Tr", "Ts", "Tm", "Td", "TD", "T*",
    # A clipping path may be part of the XObject's isolated placement block.
    "m", "l", "c", "v", "y", "h", "re", "W", "W*", "n",
    # Marked-content points do not paint page content.
    "MP", "DP", "BMC", "BDC", "EMC",
}


def _block_only_serves_xobject(
    instructions: list,
    q_index: int,
    Q_index: int,
    do_index: int,
    *,
    nested_span: tuple[int, int] | None = None,
) -> bool:
    """Return True when a q/Q block contains one XObject call plus setup/clip ops only."""
    for index in range(q_index + 1, Q_index):
        if index == do_index:
            continue
        if nested_span is not None and nested_span[0] <= index < nested_span[1]:
            continue
        operator = str(instructions[index].operator)
        if operator == "Do" or operator not in _XOBJECT_WRAPPER_NONPAINTING_OPS:
            return False
    return True

def _enclosing_q_block_around_range(
    instructions: list, start: int, end: int
) -> tuple[int, int] | None:
    """Find the nearest q/Q pair outside an already balanced [start, end) range."""
    depth = 0
    q_index = -1
    ...
    depth = 0
    for index in range(end, len(instructions)):
        operator = str(instructions[index].operator)
        if operator == "q":
            depth += 1
        elif operator == "Q":
            if depth == 0:
                return q_index, index
            depth -= 1
    return None


def _xobject_span_bounds(instructions: list, do_index: int) -> tuple[int, int]:
    """
    Return a page-space-safe span for one XObject ``Do`` invocation.

    Prefer the complete isolated q/Q placement block so an outer edit transform
    also transforms its placement ``cm``, clipping path, and local graphics
    state. If no isolated block exists, include the contiguous placement ``cm``
    run immediately preceding ``Do``. The range always identifies this single
    invocation; it never groups two calls merely because their bboxes match.
    """
    block = _enclosing_q_block(instructions, do_index)
    if block is None:
        return _preceding_cm_run_start(instructions, do_index), do_index + 1

    q_index, Q_index = block
    if not _block_only_serves_xobject(instructions, q_index, Q_index, do_index):
        return _preceding_cm_run_start(instructions, do_index), do_index + 1

    start, end = q_index, Q_index + 1
    # Include outer clip/placement wrappers when they also serve only this XObject.
    while True:
        outer = _enclosing_q_block_around_range(instructions, start, end)
        if outer is None:
            break
        outer_q, outer_Q = outer
        if not _block_only_serves_xobject(
            instructions,
            outer_q,
            outer_Q,
            do_index,
            nested_span=(start, end),
        ):
            break
        start, end = outer_q, outer_Q + 1
    return start, end
...
def _image_span_bounds(instructions: list, do_index: int) -> tuple[int, int]:
    ...
    if marked_clip is not None:
        ...
        return marked_clip
    # Design tools commonly emit an image as:
    #   q <clip path> W n q <gs/cm> /Im Do Q Q
    # The clip is page-stream state, not part of the Image XObject. Treat every
    # isolated setup/clip wrapper as part of the same atomic invocation so
    # move/affine/delete/visibility cannot leave the clip behind.
    return _xobject_span_bounds(instructions, do_index)
    ...
    for i, instr in enumerate(instructions):
        ...
        if op == "Do":
            ...
            if name is not None:
                ...
                if subtype == "Form":
                    geometry = _xobject_form_geometry(page, name)
                    if geometry is not None:
                        form_bbox, form_matrix = geometry
                        # PDF transformation order: form-space -> Form /Matrix ->
                        # current page CTM at the Do invocation.
                        form_ctm = mult_matrix(form_matrix, ctm)
                        x0, y0, x1, y1 = form_bbox
                        corners = [
                            apply_matrix([x0, y0], form_ctm),
                            apply_matrix([x1, y0], form_ctm),
                            apply_matrix([x0, y1], form_ctm),
                            apply_matrix([x1, y1], form_ctm),
                        ]
                        start_idx, end_idx = _xobject_span_bounds(instructions, i)
                        spans.append(
                            OpSpan(
                                start=start_idx,
                                end=end_idx,
                                ...
                                ocgIds=sorted(
                                    set(_active_ocg_ids())
                                    | _xobject_ocg_ids(page, name)
                                ),
                            )
                        )
                elif subtype == "Image" or subtype is None:
...
def map_object_spans(
    ...
    *,
    ...
    # A Form XObject span represents exactly one page-level Do invocation.
    # Never use the fill+stroke same-bbox grouping rule for Forms: two calls of
    # one Form (or two Forms with identical BBoxes) are distinct page objects.
    # drawIndex may disambiguate their drawing order; otherwise fail safely.
    if any(candidate.kind == "vector" and candidate.resource_name for candidate in candidates):
        span = map_object(
            page,
            obj_meta,
            pdf=pdf,
            allow_same_bbox_order=True,
            prebuilt_spans=spans,
        )
        return [span] if span is not None else []
...
def text_show_op_for_move(page, obj_meta, pdf: pikepdf.Pdf | None = None) -> dict | None:
    ...
    try:
        from app.core.edit_debug_log import log_text_move
        log_text_move(
            "text.move.map_show",
            metaId=_meta_field(obj_meta, "id"),
            metaBbox=b,
            targetIndex=best["index"],
            bestArea=best_area,
            centerMatchCount=len(center_cands),
            poolSize=len(pool),
            ...
            topScores=[
                {
                    "i": d["index"],
                    "area": area,
                    "showBbox": normalize_bbox(d["bbox"]),
                }
                for area, d in scored[:5]
            ],
        )
    except Exception:
        pass
