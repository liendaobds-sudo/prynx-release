...
pub fn staggered_hex(
    ...
    if item_w <= TOL || item_h <= TOL {
        return empty();
    }
    if usable_w < item_w - TOL || usable_h < item_h - TOL {
        return empty();
    }
    ...
    if step_y <= TOL && usable_h < item_h - TOL {
        return empty();
    }
    ...
    for row in 0..max_rows {
        ...
        if cy + ry > usable_h + TOL {
            break;
        }

        let is_odd = row % 2 != 0;
        let row_start = if is_odd {
            rx + item_w / 2.0 + gap_x / 2.0
        } else {
            rx
        };
        ...
        if is_odd {
            if usable_w >= row_start - rx + item_w - TOL {
                ...
                if step_x > TOL {
                    ...
                    if rem >= -TOL {
                        n += ((rem + TOL) / step_x).floor() as usize;
                    }
                }
            }
...
pub fn staggered_vertical(
    ...
    if step_x <= TOL && usable_w < item_w - TOL {
        return empty();
    }
    ...
    for col in 0..max_cols {
        ...
        let is_odd = col % 2 != 0;
        let col_start = if is_odd {
            ry + item_h / 2.0 + gap_y / 2.0
        } else {
            ry
        };
        ...
        if is_odd {
            if usable_h >= col_start - ry + item_h - TOL {
                ...
                if step_y > TOL {
                    ...
                    if rem >= -TOL {
                        n += ((rem + TOL) / step_y).floor() as usize;
                    }
                }
            }
...
pub fn cluster_grid(
    ...
    let cols = if usable_w >= cluster_w - TOL {
        ((usable_w - cluster_w + TOL) / dx_outer).floor() as usize + 1
    } else {
        0
    };
    let rows = if usable_h >= cluster_h - TOL {
        ((usable_h - cluster_h + TOL) / dy_outer).floor() as usize + 1
    } else {
        0
    };
...
pub fn row_alternating(
    ...
    if step_x <= TOL || row_h <= TOL {
        return empty();
    }

    let max_rows = if usable_h >= h - TOL {
        ((usable_h - h + TOL) / row_h).floor() as usize + 1
    } else {
        0
    };
    ...
    for r in 0..max_rows {
        ...
        let cols = if usable_w >= x_start + w - TOL {
            ((usable_w - x_start - w + TOL) / step_x).floor() as usize + 1
        } else {
            0
        };
...
pub fn col_alternating(
    ...
    if col_w <= TOL || step_y <= TOL {
        return empty();
    }

    let max_cols = if usable_w >= w - TOL {
        ((usable_w - w + TOL) / col_w).floor() as usize + 1
    } else {
        0
    };
    ...
    for c in 0..max_cols {
        ...
        let rows = if usable_h >= y_start + h - TOL {
            ((usable_h - y_start - h + TOL) / step_y).floor() as usize + 1
        } else {
            0
        };
...
mod tests {
    ...
    fn cluster_has_rotated_180() {
        ...
        assert!(
            r.items.iter().any(|it| it.is_rotated_180),
            "cluster phải có item xoay 180"
        );
        within_bounds(&r, 320.0, 450.0);
    }
    ...
    fn empty_when_item_too_large() {
        assert_eq!(
            staggered_hex(30.0, 30.0, 50.0, 50.0, 0.0, 0.0).total_items,
            0
        );
        assert_eq!(
            solve_grid(100.0, 100.0, 200.0, 200.0, 0.0, 0.0).total_items,
            0
        );
    }
