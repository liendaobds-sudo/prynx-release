...
pub fn validate_request_json(request_json: &str) -> Result<Value, String> {
    ...
    for key in NUMERIC_PARAMS {
        number_in(params.get(*key), &format!("params.{key}"), 0.0, 10_000.0)?;
    }
    ...
    one_of(
        params.get("panelOrder"),
        "params.panelOrder",
        &["WLWL", "LWLW"],
    )?;
    one_of(
        params.get("handleShape"),
        "params.handleShape",
        &["oval", "roundRect"],
    )?;
    one_of(
    ...
    one_of(
        params.get("gableStyle"),
        "params.gableStyle",
        &["flat", "pitched"],
    )?;
    one_of(
        params.get("cupHeightType"),
        "params.cupHeightType",
        &["slant", "vertical"],
    )?;
    one_of(
        params.get("cupFlapPosition"),
        "params.cupFlapPosition",
        &["right", "left", "none"],
    )?;
    one_of(
        params.get("envFlapShape"),
        "params.envFlapShape",
        &["straight", "pointed", "rounded"],
    )?;
    one_of(
        params.get("envStyle"),
        "params.envStyle",
    ...
    for (label, value) in [
        ("sheet.width", sheet.get("width")),
        ("sheet.height", sheet.get("height")),
        ("sleeveSheet.width", sleeve.get("width")),
        ("sleeveSheet.height", sleeve.get("height")),
    ] {
    ...
    one_of(
        nesting.get("sheetOrientation"),
        "sheetOrientation",
        &["auto", "portrait", "landscape"],
    )?;
    one_of(
        nesting.get("nestingMode"),
        "nestingMode",
        &["grid", "smart"],
    )?;
    one_of(
        nesting.get("trayNestingMode"),
        "trayNestingMode",
        &["combined", "split"],
    )?;
    Ok(root)
...
mod tests {
    ...
    fn fixture_has_hanging_window_params() {
        ...
        for key in ["WNW", "WNH", "HTH", "ABD"] {
            assert!(
                params.get(key).is_some_and(|v| v.is_number()),
                "thiếu số {key}"
            );
        }
        assert!(
