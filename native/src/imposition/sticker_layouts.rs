...
pub(crate) fn result_to_pydict(
    ...
    if let Some(c) = r.cols {
        d.set_item("cols", c)?;
    }
    if let Some(rr) = r.rows {
        d.set_item("rows", rr)?;
    }
    Ok(d.into())
}
...
#[pyfunction]
pub fn sticker_staggered_vertical(
    py: Python<'_>,
    usable_w: f64,
    usable_h: f64,
    item_w: f64,
...
#[pyfunction]
#[pyo3(signature = (usable_w, usable_h, item_w, item_h, gap_x, gap_y, params, rotated))]
pub fn sticker_row_alternating(
    py: Python<'_>,
    usable_w: f64,
    usable_h: f64,
    item_w: f64,
    ...
    let r = core::row_alternating(
        usable_w, usable_h, item_w, item_h, gap_x, gap_y, rp, rotated,
    );
    result_to_pydict(py, &r, w, h)
}
...
#[pyfunction]
#[pyo3(signature = (usable_w, usable_h, item_w, item_h, gap_x, gap_y, params, rotated))]
pub fn sticker_col_alternating(
    py: Python<'_>,
    usable_w: f64,
    usable_h: f64,
    item_w: f64,
    ...
    let r = core::col_alternating(
        usable_w, usable_h, item_w, item_h, gap_x, gap_y, cp, rotated,
    );
    result_to_pydict(py, &r, w, h)
}
