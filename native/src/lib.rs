...
fn pdfcompare_native(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_class::<print_engine_py::PpeRenderSession>()?;
    m.add_function(wrap_pyfunction!(print_engine_py::ppe_separations, m)?)?;
    m.add_function(wrap_pyfunction!(
        print_engine_py::ppe_compose_separation_subset,
        m
    )?)?;
    ...
    m.add_function(wrap_pyfunction!(
        combine_image_pdf::combine_image_manifest_native,
        m
    )?)?;
    ...
    m.add_function(wrap_pyfunction!(
        imposition::grid_solver::solve_optimal_layout,
        m
    )?)?;
    m.add_function(wrap_pyfunction!(imposition::grid_solver::solve_manual, m)?)?;
    m.add_function(wrap_pyfunction!(
        imposition::grid_solver::get_src_page_idx,
        m
    )?)?;

    // Imposition assembler
    m.add_function(wrap_pyfunction!(
        imposition::assembler::compute_placements,
        m
    )?)?;
    m.add_function(wrap_pyfunction!(
        imposition::assembler::compute_alignment,
        m
    )?)?;
    m.add_function(wrap_pyfunction!(
        imposition::assembler::compute_mark_coords,
        m
    )?)?;

    // Sticker layout solvers
    m.add_function(wrap_pyfunction!(
        imposition::sticker_layouts::sticker_solve_grid,
        m
    )?)?;
    m.add_function(wrap_pyfunction!(
        imposition::sticker_layouts::sticker_staggered_hex,
        m
    )?)?;
    m.add_function(wrap_pyfunction!(
        imposition::sticker_layouts::sticker_staggered_vertical,
        m
    )?)?;
    m.add_function(wrap_pyfunction!(
        imposition::sticker_layouts::sticker_hex_tiling_row,
        m
    )?)?;
    m.add_function(wrap_pyfunction!(
        imposition::sticker_layouts::sticker_hex_tiling_col,
        m
    )?)?;
    m.add_function(wrap_pyfunction!(
        imposition::sticker_layouts::sticker_cluster_grid,
        m
    )?)?;
    m.add_function(wrap_pyfunction!(
        imposition::sticker_layouts::sticker_row_alternating,
        m
    )?)?;
    m.add_function(wrap_pyfunction!(
        imposition::sticker_layouts::sticker_col_alternating,
        m
    )?)?;

    // Shape-specific solvers
    m.add_function(wrap_pyfunction!(
        imposition::shape_solvers::shape_pointy_hex,
        m
    )?)?;
    m.add_function(wrap_pyfunction!(
        imposition::shape_solvers::shape_flat_hex,
        m
    )?)?;
    m.add_function(wrap_pyfunction!(
        imposition::shape_solvers::shape_trapezoid,
        m
    )?)?;
    m.add_function(wrap_pyfunction!(
        imposition::shape_solvers::shape_triangle,
        m
    )?)?;
    m.add_function(wrap_pyfunction!(
        imposition::shape_solvers::shape_pentagon,
        m
    )?)?;
    m.add_function(wrap_pyfunction!(
        imposition::shape_solvers::shape_parallelogram,
        m
    )?)?;
    m.add_function(wrap_pyfunction!(
        imposition::shape_solvers::shape_l_layout,
        m
    )?)?;
    m.add_function(wrap_pyfunction!(
        imposition::shape_solvers::shape_hammer,
        m
    )?)?;
    m.add_function(wrap_pyfunction!(
        imposition::shape_solvers::shape_dumbbell_pair_col,
        m
    )?)?;
    m.add_function(wrap_pyfunction!(
        imposition::shape_solvers::shape_dumbbell_pair_row,
        m
    )?)?;

    // Orchestrator
    m.add_function(wrap_pyfunction!(
        imposition::orchestrator::generate_layout_candidates,
        m
    )?)?;

    // N-Up cắt xén chia tỷ lệ (ratio_stack) — Phase 1
    m.add_function(wrap_pyfunction!(
        imposition::ratio_stack_solver::solve_ratio_stack,
        m
    )?)?;
