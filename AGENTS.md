...
Codex: skill tự nạp theo description, hoặc gọi tường minh `$prynx-dieline`. Claude Code: tự nạp, hoặc `/audit`, `/add-boxtype`, `/verify` (xem `.claude/commands/`).
...
3. **PDFium không thread-safe.** Mọi truy cập native ↔ pypdfium2 phải qua `PDFIUM_PY_LOCK` trong `backend/app/core/rust_bridge.py`; sửa `pdf_processor` thì sửa `rust_bridge` cùng lúc.
